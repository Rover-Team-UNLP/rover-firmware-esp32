...
static void process_error(const Error_inf *error)
{
    ...
    if (severity >= SEVERITY_WARNING)
    {
        ...
        if (from_error_queue != NULL)
        {
            ...
            {
                ESP_LOGW(TAG, "Could not send error to from_error_queue (queue full or timeout)");
            }
        }
    }
    ...
    case WEBSOCKET_DISCONNECTED:
        ESP_LOGW(TAG, "WebSocket disconnected - camera may remain active");
        break;

    case CAMERA_INIT_FAIL:
    case WEBSOCKET_INIT_FAIL:
    case WEBSOCKET_START_FAIL:
        ESP_LOGE(TAG, "Critical error: %s (id=%u)", error_name, error->id);
        break;
...
static void error_control_task(void *pvParameters)
{
    ...
    ESP_LOGI(TAG, "Error control task started, waiting on to_error_queue");
...
esp_err_t error_control_init(void)
{
    ...
    {
        ESP_LOGE(TAG, "error_control_init: to_error_queue is NULL (call after init_queues)");
        return ESP_FAIL;
    }

    if (from_error_queue == NULL)
    {
        ESP_LOGW(TAG, "error_control_init: from_error_queue is NULL, errors will not be sent over WebSocket");
    }
    ...
    ESP_LOGI(TAG, "Error control module ready");
    return ESP_OK;
}
...
void error_control_stop(void)
{
    if (error_task_handle != NULL)
    {
        ...
        ESP_LOGI(TAG, "Error control stopped");
    }
}
...
void error_control_set_camera_state(camera_state_t state)
{
    ...
    {
        ESP_LOGI(TAG, "Camera state set manually to %d", (int)state);
